...
export default function Inventory() {
    ...
    // Adjust modal state — null when closed, inventory item when open
...
const canAdjust = user.role !== 'store_staff'
    ...
    // Preview of what the stock will be after the operation
    const previewNewQty = () => {
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                <table className="w-full">
                    <thead className="bg-gray-800">
                        <tr>
                            {['Product', 'Warehouse', 'Quantity', 'Threshold', 'Status', 'Actions'].map(h => (
                    ...
                    <tbody className="divide-y divide-gray-800">
                        {inventory.map(item => (
                            <tr key={item.id} className={`transition-colors hover:bg-gray-800/50 ${item.low_stock ? 'bg-red-500/5' : ''}`}>
                                <td className="px-4 py-3 text-white text-sm font-medium">{item.product_name}</td>
                                <td className="px-4 py-3 text-gray-400 text-sm">{item.warehouse_name}</td>
                                <td className="px-4 py-3 text-white text-sm font-semibold">{item.quantity}</td>
                                <td className="px-4 py-3 text-gray-400 text-sm">{item.threshold}</td>
