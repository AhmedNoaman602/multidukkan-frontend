...
export default function CustomerBalance() {
    ...
    const [mode, setMode] = useState(null) // null | 'payment' | 'credit'
    ...
    const fetchData = async () => {
        try {
            ...
            // Only show unpaid orders for this customer
            setOrders(
                ordersRes.data.data.filter(o =>
                    o.customer_id === parseInt(id) && o.status === 'unpaid'
                )
            )
        } catch (err) {
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                       <button
                        ...
                        >
                            {mode === 'auto' ? 'Cancel' : '💰 Receive Payment'}
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div className="bg-gray-900 border border-purple-500/20 rounded-xl p-6 mb-6">
        <h3 className="text-white font-semibold mb-1">💰 Receive Payment</h3>
        <p className="text-gray-500 text-xs mb-4">
            Pays unpaid orders oldest first. Excess becomes credit for future orders.
        </p>

        {/* Order preview — only when orders exist */}
        {orders.length === 0 ? (
            <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <p className="text-blue-400 text-sm">No unpaid orders. Full amount will be stored as credit.</p>
            </div>
        ) : (
        ...
        <form onSubmit={handleAutoPayment} className="grid grid-cols-2 gap-4">
            <div>
