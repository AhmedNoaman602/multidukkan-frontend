...
export default function PurchaseOrders() {
  ...
  const handlePay = async (e) => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.message || t("orders.payModal.failed"),
        "error",
      );
    } finally {
      setPaying(false);
    ...
    <div>
      <div className="mb-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <h2 className="text-2xl font-bold text-white">{t("purchaseOrders.title")}</h2>
          <button
      ...
      {stats && (
        <StatBoxes
          stats={[
            {
              label: t("purchaseOrders.totalOrders"),
              value: formatNumber(stats.total_orders),
              color: "white",
            },
            {
              label: t("purchaseOrders.totalSpent"),
              value: formatCurrency(stats.total_spent, lang),
              color: "white",
            },
            {
              label: t("orders.list.paidAmount"),
              value: formatCurrency(stats.paid_amount, lang),
              color: "green",
            },
            {
              label: t("orders.list.unpaidAmount"),
              value: formatCurrency(stats.unpaid_amount, lang),
              color: "red",
            },
          ]}
        />
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-x-auto">
            ...
            {purchaseOrders.map((order) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-gray-400 text-sm">
                  {t("purchaseOrders.itemsCount", { count: order.items_count })}
                </td>
        ...
        {purchaseOrders.length === 0 && (
          <div className="text-center py-16 text-gray-500">
            {hasActiveFilters
              ? t("purchaseOrders.emptyFiltered")
